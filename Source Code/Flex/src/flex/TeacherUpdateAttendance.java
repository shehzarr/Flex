 ...
 * @author Danish Ali
 */
public class TeacherUpdateAttendance extends javax.swing.JFrame {
    ...
    public TeacherUpdateAttendance() throws HeadlessException {
    }

    public TeacherUpdateAttendance(String course, String section, String lecno) {
        initComponents();
        setSize(900, 500);
...
//        setTitle("Add Attendance");
        ...
        List<Attendance> currAttendances = new ArrayList<>();
        for (int i = 0; i < std.size(); i++) {
            obj.getAttendanceByLecture(std.get(i).rollNo, course, currAttendances, lecno);
        }
        ...
        update = createbutton("Update", 250, row, true);
        row = row + 40;
        data.add(back);
        data.add(update);
...
//        for (int i = 0; i < 101; i++) {
        for (int i = 0; i < std.size(); i++) {
            ...
            if (currAttendances.get(i).status.equals("-")) {
                int j = 0;
                status[j] = present;
                j++;
                status[j] = absent;
                j++;
                status[j] = late;
            } else if (currAttendances.get(i).status.equals("A")) {
                int j = 0;
                status[j] = absent;
                j++;
                status[j] = present;
                j++;
                status[j] = late;
            } else if (currAttendances.get(i).status.equals("L")) {
                int j = 0;
                status[j] = late;
                j++;
                status[j] = absent;
...
//            data.add(createLabel(temp, 10, row, true));
        ...
        update.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                obj.update_Attendance(attendanceList, course, section, date.getText(), lecno);
                new TeacherGUI(TeacherGUI.teacherName, TeacherGUI.teacherId).setVisible(true);
            }
        });
    }
...
//        panel.add(but);
    ...
    private void initComponents() {
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGap(18, 18, 18)
                .addComponent(teacheraddattendance, javax.swing.GroupLayout.DEFAULT_SIZE, 199, Short.MAX_VALUE)
                .addContainerGap())
        );
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TeacherUpdateAttendance.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TeacherUpdateAttendance().setVisible(true);
            }
        });
    }
